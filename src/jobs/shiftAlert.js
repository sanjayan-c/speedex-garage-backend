...
export function getLastAlertIfFresh(maxAgeSec = 120) {
  if (!lastAlertPayload || !lastAlertAt) return null;
  const age = (Date.now() - lastAlertAt.getTime()) / 1000;
  return age <= maxAgeSec ? lastAlertPayload : null;
}
  ...
  pingTask = cron.schedule(
    ...
    async () => {
      try {
        ...
        console.log(
          `[cron] shift-alert TICK @ ${torNow.toISO()} | window ${start.toFormat("HH:mm")}–${end.toFormat("HH:mm")} (end+${marginMinutes}m) | inWindow=${inWindow}`
        );
        ...
        const payload = {
          type: "shift-alert",
          end_local_time: marginEndText, // now counts down to (end + margintime)
          timezone: "America/Toronto",
          alert_minutes: alertMinutes,   // how early before margin-end
          // Optional: include meta for debugging/FE display if you want
        ...
        console.log(
          `[cron] shift-alert EMIT (minute ping) | sockets=${total} roomSize=${room ? room.size : 0} | target=${marginEndText} (end+${marginMinutes}m)`
        );
...
export async function isWithinAlertWindowToronto() {
  ...
  const torNow = DateTime.now().setZone("America/Toronto");
  const inWindow = torNow >= start && torNow <= end;
  return { inWindow, end_local_time: marginEndText, alert_minutes: alertMinutes, margin_minutes: marginMinutes };
}
