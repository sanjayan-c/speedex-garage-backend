...
const weekTimeArray = Joi.array()
  .length(7)
  .items(timeString.allow(null))
  .messages({
    "array.length": "Shift arrays must contain exactly 7 elements (Mon..Sun).",
  });
...
const staffCreateSchema = Joi.object({
  ...
  shiftStart: weekTimeArray.required(),
  shiftEnd: weekTimeArray.required(),
...
const shiftUpdateSchema = Joi.object({
  ...
  marginTime: Joi.number()
    .integer()
    .min(0)
    .max(24 * 60)
    .optional(),
  alertTime: Joi.number()
    .integer()
    .min(0)
    .max(24 * 60)
    .optional(),
